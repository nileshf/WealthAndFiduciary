...
$TaskFile = Join-Path $ServiceDir '.kiro/specs/security-service/project-task.md'
$EnvFile = Join-Path $ServiceDir '.env'

Write-Host "SecurityService Jira Sync" -ForegroundColor Cyan
...
foreach ($task in $tasksToCreate) {
    ...
    $issueKey = New-JiraIssue -Summary $task.Name -Description "SecurityService task: $($task.Name)"
    ...
    if ($issueKey) {
        ...
        $labelsUpdated = Set-JiraIssueLabels -IssueKey $issueKey -Labels @('ai-security-service')
