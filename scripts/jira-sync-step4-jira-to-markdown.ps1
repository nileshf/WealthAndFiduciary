...
.EXAMPLE
    .\scripts\jira-sync-step4-jira-to-markdown.ps1                    # Run for all services
    .\scripts\jira-sync-step4-jira-to-markdown.ps1 -ServiceName "SecurityService"  # Run for one service
#>
...
Write-Host "           Step 4: Sync Status Changes from Jira               " -ForegroundColor Cyan
...
foreach ($service in $services) {
    ...
    $result = & .\scripts\jira-sync-step4-logic.ps1
    if ($result -ne 0) {
        Write-Host "Step 4 failed with exit code: $result" -ForegroundColor Red
        $failureCount++
        continue
    }
